        ...
        for( int i = 0; i < boardSize.height; i++ )
            for( int j = 0; j < boardSize.width; j++ )
                corners.push_back(Point3f(float(i*squareSize),
                                          float(j*squareSize), // j-index becomes y-coordinate
                                           0));
                           ...
                           double& totalAvgErr)
{
    ...
    // determine the coordinates of the 3D object points
    calcChessboardCorners(boardSize, squareSize, objectPoints[0], patternType);
    
    // this assumes that only one calibration pattern is used during the capture.
    //
...
int main( int argc, char** argv )
{
    ...
    if( inputFilename ) {
        if( !videofile && readStringList(inputFilename, imageList) ) {
            mode = CAPTURING;
            // this mode is when image list is supplied on the command line.
        }
        else
    ...
    cerr << (mode == CAPTURING ? "mode = CAPTURING" : "mode = ????") << endl;
    for(i = 0;;i++)
        {
        Mat view, viewGray;
        bool blink = false;
        
        if( capture.isOpened() ) /* video */{
            Mat view0;
            capture >> view0;
            view0.copyTo(view);
            }
        else if( i < (int)imageList.size() ) // read an image from the filelist
            view = imread(imageList[i], 1);
        
        cerr << " --- " << endl << "if (view.empty()) : " << (view.empty() ? "TRUE" : "FALSE") << endl << endl;
        
        if(view.empty())
            {
            cerr << "VIEW EMPTY" << endl;
        ...
        switch( pattern )
            {
                case CHESSBOARD:
                //
                // all of the corners are found and they are placed in a certain order (row by row, left to right in every row)
                //
                ...
                cerr << i
                << "  " << (found? "TRUE":"FALSE") << " findChessboardCorners( view, boardSize, pointbuf, CALIB_CB_ADAPTIVE_THRESH | CALIB_CB_FAST_CHECK | CALIB_CB_NORMALIZE_IMAGE);" << endl;
                break;
        ...
        // it's been done in cv::findChessboardCorners().
        // you can try with other parameters.
        if( pattern == CHESSBOARD && found) {
//            cv::cornerSubPix( viewGray, pointbuf, Size(11,11), Size(-1,-1),
//                             TermCriteria( TermCriteria::EPS+TermCriteria::COUNT, 30, 0.1 ));
        ...
        if(found) {
            cv::drawChessboardCorners( view, boardSize, Mat(pointbuf), found );
            for (int i=0; i<pointbuf.size(); i++)
                cv::putText(view, string(to_string(i)), pointbuf[i], 1, 1, cv::Scalar(0,255,255));
        }
        ...
        imshow(std::string(std::to_string(i) + " Image View"), view);
        int key = 0xff & waitKey(capture.isOpened() ? 50 : 100);
        ...
        if( mode == CAPTURING && imagePoints.size() >= (unsigned)nframes )
            {
            cerr << "---- do camera calibration ----" << endl;
    ...
    cerr << "if (showUndistorted): " << (showUndistorted? "TRUE":"FALSE") << endl;
...
//    if( !capture.isOpened() && showUndistorted )
        ...
        for( i = 0; i < (int)imageList.size(); i++ )
            ...
            imshow(string(to_string(i) + ":: Image View with coordinates, XYZ=RGB"), rview);
